...
1. Push this folder to GitHub, e.g. `github.com/alex-m1/appscan360-gitlab`. If your repo has another name run `./set-origin.sh <owner/repo> [tag]` first: it rewrites every URL in the templates and pipeline file.
...
4. Projects need **nothing copied**. Either
   * add one line to the project's `.gitlab-ci.yml` (see `examples/project-gitlab-ci.yml`):
     `include: - remote: 'https://raw.githubusercontent.com/alex-m1/appscan360-gitlab/main/yaml/appscan360_scan_sast.yaml'`
